...
int main(int argc,char *argv[]) {
    ...
    if (system("net session 2> Nul 1> Nul") != 0) {
        system(ws2c(L"start /b cmd.exe /r \"ping 0.0.0.0 -n 1 -w 100 > Nul & powershell start -verb runas '"+s2ws(argv[0])+L"' > Nul\""));
        _exit(0);
    }

    system("if not exist bin\\ mkdir bin");

    list<string> const files = {"bin/devcon.exe","bin/mouselock.ahk","bin/keyboardlock.ahk","bin/taskmanagerassasin.exe","bin/nircmd.exe","bin/hard-reset.bat"};
    struct stat buffer;
    for (list<string>::const_iterator i = files.begin(); i != files.end(); ++i) {
        if (!(stat (i->c_str(), &buffer) == 0)) {
            system(ws2c(L"curl -l \"https://raw.githubusercontent.com/reedthorngag/device-lock/main/" + s2ws(i->c_str()) + L"\" --output " + s2ws(i->c_str())));
        }
    }
    ...
    fstream file;
    string password;
    file.open("bin\\password.txt",ios::in);
    if (file.is_open()) {
        ...
        if (password=="") {
            ...
            cin >> password;
            file.open("bin\\password.txt",ios::out);
            file << password << flush;
            file.close();
        }
    } else {
    ...
    system("start AutoHotkey bin\\mouselock.ahk > Nul");
